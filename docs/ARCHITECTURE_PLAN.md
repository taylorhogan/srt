...
| **2** | **Guards centralized; conductor owns the roof.** All six enforcement sites rewired through `POST /v1/events`; conductor authoritative *for the roof only*. `end.py` fallback added. `FAULT_ROOF_UNKNOWN` real and persistent. **Roof motor current is informational only** (owner, 2026-09-16): the signature-vs-golden verdict (breakaway peak, drift) posts to chat/phone and the morning report but never gates a move, a night, or the CLEAN verdict; only the sustained-draw stall watchdog (`ROOF_STALL`, motor still pulling >20 W past 30 s) cuts power and faults. **Progress 2026-09-16 (2a, shipped dark behind `conductor.roof_authority`):** every relay site asks `POST /v1/events` with sensed evidence (`iris/client.py`) and reports after; manual roof states + `ROOF_*_REQUESTED`; unplanned `image!!` opens from `IDLE_DAY`; motion watchdog → `ROOF_TIMEOUT`; restart mid-move → FAULT; `end.py` fallback + marker; `resolve!`. **AUTHORITY ON 2026-09-17 (7d4617d):** `roof_authority` True after the supervised daytime cycle (roof!! open 09:26 / close 09:30, both allowed) and the refusal drill (`scripts/conductor_drill.py`): in a hold, `roof!! open` was refused and nothing moved (no relay, no motor plug, roof still shut by vision). The same morning showed the cost of authority: sun glare hid the open star, the 10:14 open never confirmed, ROOF_TIMEOUT → FAULT_ROOF_UNKNOWN, and a plain close was then refused (`roof_state_known`) — closed with `roof!! close force` (allowed-in-hold on asserted evidence), then resolve!. A positive OPEN sense that does not depend on the star (far-end camera reading a roof tag at both positions, and/or the open limit switch) is the fix. Still legacy: the relay fire itself (`toggle_roof`) — moving it into `iris/hardware/roof.py` with the CI grep is 2b, along with ESTOP as a conductor action and an Invariant-B guard on mount moves and mount power (on 2026-09-17 a run that missed a stop! powered the mount and launched flats while the conductor sat in SAFE_HOLD). **2b progress 2026-09-25:** stop! posts `ESTOP_REQUESTED` before it touches anything (live; the hold is left only by resolve!, and end.py's close inside it is answered on the evidence as before); the mount asks -- `MOUNT_MOVE_REQUESTED` before stop!'s park, the blind park, the NINA prelude and main sequence, `MOUNT_POWER_REQUESTED` before the prelude's start.py and flats -- answered by permission rows in the table (dst == src, Invariant B = `roof_open` on every move row, power under a shut roof needs parked+shut, no row in any hold; a park inside a hold follows the evidence or the blind-park assertion). Shipped dark behind `conductor.mount_authority` (advisory until a night of clean verdicts). Still 2b: the relay fire into `iris/hardware/roof.py` + CI grep; roof_evidence and the north camera's verdict into `SensorSnapshot` (limit switches dropped by the owner 2026-09-25). | M | Full night incl. open+close via the new path; supervised daytime cycle **MET 2026-09-17 (via roof!!)**; deliberate guard-rejection **MET 2026-09-17**; the full night under authority is what remains |
...
| **7** | **Multi-slot nights. PARTLY DELIVERED EARLY, in legacy code (2026-09).** `control/slot_plan.py` + `nina_gen.generate_slots_sequence` already fit TWO targets into the dark hours — operator pins first, a hard TimeCondition per slot so one cannot eat the next, prelude-once/slot/flats-once — and ran for real on 2026-09-11 (ngc7380 → m33, handover 03:08) and 2026-09-14. Its own docstring calls itself "the small version of Phase 7". **Remaining here:** (a) `REPLAN_REQUESTED` on weather or mid-night convergence — today's plan is frozen when the sequence file is written, and the machine has no such event; (b) more than two slots (`plan_slots` is deliberately "fit two"); (c) ownership — the plan is a NINA sequence file built by the scheduler, and on the new spine the conductor plans from the registry (need × horizon × pins) and drives slots through the Phase 6 capture seam, which is what makes (a) possible at all. | **L** → **M** | A real two-target night: **MET 2026-09-11** (both slots' frames correctly attributed in the morning report). Still owed: a night re-planned mid-flight after a weather turn or a convergence, and two pinned targets taking the slots over a higher-need unpinned one |
...
2. **Roof limit switches** — DROPPED by the owner 2026-09-25 ("I feel confident that the cameras
   are working"). The decode + commissioning tool (a5f7648) stay dormant. The second
   independent roof sense is the north camera (2026-09-24: AprilTag on the moving truss read at
   both positions, unoccludable by the scope); it and `roof_evidence` are what go into
   `SensorSnapshot` in 2b.
